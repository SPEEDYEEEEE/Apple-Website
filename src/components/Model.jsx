...
const Model = () => {
    ...
    //rotation
    const [smallRotaiton, setSmallRotation] = useState(0);
    const [largeRotaiton, setLargeRotation] = useState(0);

    const tl = gsap.timeline();

    useEffect(() => {
        if(size === 'large') {
            animateWithGsapTimeline(tl, small, smallRotaiton, '#view1', '#view2', {transform: 'translateX(-100%)', duration: 2})
        }
        if(size === 'small') {
            animateWithGsapTimeline(tl, large, largeRotaiton, '#view2', '#view1', {transform: 'translateX(0)', duration: 2})
        }
    }, [size])
                ...
                <div className='mx-auto w-full'>
                    ...
                    <div className='flex-center'>
                        <ul className='color-container'>
                            {models.map((item, i) => (
                                <li key={item} className='w-6 h-6 rounded-full mx-2 cursor-pointer' style={{backgroundColor: item.color[0]}} onClick={() => setModel(item)}></li>
                            ))}
                        </ul>
